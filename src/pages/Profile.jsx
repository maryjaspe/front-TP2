...
const Profile = () => {
  ...
  const [user, setUser] = useState({ nombre: '', email: '', direccion: '', telefono: '' });
  const [userOriginal, setUserOriginal] = useState({ direccion: '', telefono: '' });
  ...
  const handleUpdateProfile = () => {
    ...
    if (user.direccion === userOriginal.direccion && user.telefono === userOriginal.telefono) {
  ...
  return (
    <div className="flex flex-col h-screen">
      <Menu />
      <div className="p-8">
        {showChip && <div className={`chip ${isError ? 'bg-red-500' : 'bg-green-500'} text-white p-2 rounded`}>{chipMessage}</div>}
        <div className='flex-grow flex flex-col p-8 gap-8 shadow-xl'>
          {isLoading ? <p>Cargando...</p> :
            <>
              <p className="text-xl mt-4 font-bold text-center">Perfil del Usuario</p>
              <p className="text-lg font-bold">Información del Usuario</p>
              <p><span className='font-bold'>Nombre: </span> {user.nombre}</p>
              <p><span className='font-bold'>Email: </span> {user.email}</p>
              <Input label="Dirección" initialValue={user.direccion} onChange={(value) => handleInputChange(value, 'direccion')} />
              <Input label="Teléfono" initialValue={user.telefono} onChange={(value) => handleInputChange(value, 'telefono')} />
              <Button onClick={handleUpdateProfile}>Aplicar Cambios</Button>
              <Button onClick={handleDeleteAccount}>Dar de Baja la Cuenta</Button>
            </>
          }
        </div>
      </div>
